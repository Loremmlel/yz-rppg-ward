...
TrendMetricsPanel::TrendMetricsPanel(QWidget *parent) : QWidget(parent) {
    ...
    // ── 创建所有卡片 ──
    m_cardHrAvg  = new TrendCard(QStringLiteral("心率均值"),     "❤",  QStringLiteral("bpm"), QColor(0xFF, 0x52, 0x52), scrollContent);
    m_cardBrAvg  = new TrendCard(QStringLiteral("呼吸率均值"),   "🫁", QStringLiteral("Hz"),  QColor(0x21, 0x96, 0xF3), scrollContent);
    m_cardSqiAvg = new TrendCard(QStringLiteral("信号质量均值"), "📶", QString{},             QColor(0x7E, 0x57, 0xC2), scrollContent);

    m_cardSdnn  = new TrendCard(QStringLiteral("SDNN 中位数"),  "📊", QStringLiteral("ms"), QColor(0x26, 0xA6, 0x9A), scrollContent);
    m_cardRmssd = new TrendCard(QStringLiteral("RMSSD 中位数"), "📊", QStringLiteral("ms"), QColor(0x26, 0xA6, 0x9A), scrollContent);
    m_cardSdsd  = new TrendCard(QStringLiteral("SDSD 中位数"),  "📊", QStringLiteral("ms"), QColor(0x26, 0xA6, 0x9A), scrollContent);
    m_cardPnn50 = new TrendCard(QStringLiteral("pNN50 中位数"), "📈", QString{},            QColor(0x00, 0x96, 0x88), scrollContent);
    m_cardPnn20 = new TrendCard(QStringLiteral("pNN20 中位数"), "📈", QString{},            QColor(0x00, 0x96, 0x88), scrollContent);

    m_cardLfHfRatio = new TrendCard(QStringLiteral("LF/HF 比值"), "⚖", QString{},  QColor(0xFF, 0x98, 0x00), scrollContent);
    m_cardHf        = new TrendCard(QStringLiteral("HF 均值"),    "〰", QString{},  QColor(0x66, 0xBB, 0x6A), scrollContent);
    m_cardLf        = new TrendCard(QStringLiteral("LF 均值"),    "〰", QString{},  QColor(0xFF, 0xB3, 0x00), scrollContent);
    m_cardVlf       = new TrendCard(QStringLiteral("VLF 均值"),   "〰", QString{},  QColor(0xEF, 0x53, 0x50), scrollContent);
    m_cardTp        = new TrendCard(QStringLiteral("总功率均值"), "⚡", QString{},  QColor(0x78, 0x90, 0x9C), scrollContent);

    // ── 在 scrollContent 内布局三个分组 ──
